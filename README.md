...
For detailed instructions on updating configuration, see **[CONFIG_GUIDE.md](CONFIG_GUIDE.md)**.

Quick steps:
1. Edit `config/move2germany_tasks_v1.json`
2. Add your task following the structure above
3. Run `npm run build` to validate
4. Deploy - Tasks will be available immediately
...
- Complete account deletion with GDPR-compliant data cleanup
...
- Account migration system for legacy users

### Database Security
- Row Level Security (RLS) enabled on all tables
- Users can only access their own data
- Restrictive policies by default
- Optimized auth.uid() checks for performance
...
- Full cascade delete on account deletion
- All related data removed (tasks, documents, conversations, messages)
- Audit logs anonymized (user_id set to null)
- EU-based data storage (Supabase EU region)

### Audit Logging
- All critical actions logged: USER_SIGNUP, USER_LOGIN, TASK_STATUS_CHANGED, DOCUMENT_UPLOADED, AI_TOOL_CALLED, USER_ACCOUNT_DELETED
- No sensitive data in audit logs
- Audit trail preserved even after account deletion (anonymized)
